...
void list_print(list_t *list, char sep, char end)
{
    ...
    if (cur_node->data.count == 1)
    {
        if (cur_node->next == NULL)
            printf("%d, %zu", cur_node->data.letter, cur_node->data.count);
        else
            printf("%d, %zu%c", cur_node->data.letter, cur_node->data.count, sep);
    }
    else
    {
        if (cur_node->next == NULL)
            printf(ANSI_COLOR_GREEN"%d, %zu"ANSI_COLOR_RESET, cur_node->data.letter, cur_node->data.count);
        else
            printf(ANSI_COLOR_GREEN"%d, %zu%c"ANSI_COLOR_RESET, cur_node->data.letter, cur_node->data.count, sep);
    }
    printf("%c", end);
}

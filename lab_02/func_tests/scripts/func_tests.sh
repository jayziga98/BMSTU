...
counter=1
correct_tests=0
file="./func_tests/data/pos_0${counter}"
while [ -f "${file}_in.txt" ]; do

    exit_code=1

    if [ -f "${file}_args.txt" ]; then
        bash ./func_tests/scripts/pos_case.sh "${file}_in.txt" "${file}_out.txt" "${file}_args.txt"
        exit_code=$?
    else
        bash ./func_tests/scripts/pos_case.sh "${file}_in.txt" "${file}_out.txt"
        exit_code=$?
    fi
    ...
    if [ $counter -gt 9 ]; then
        file="./func_tests/data/pos_${counter}"
    fi
done
...
counter=1
correct_tests=0
file="./func_tests/data/neg_0${counter}"
while [ -f "${file}_in.txt" ]; do

    exit_code=1

    if [ -f "${file}_args.txt" ]; then
        bash ./func_tests/scripts/neg_case.sh "${file}_in.txt" "${file}_args.txt"
        exit_code=$?
    else
        bash ./func_tests/scripts/neg_case.sh "${file}_in.txt"
        exit_code=$?
    fi
    ...
    file="./func_tests/data/neg_0${counter}"
    if [ $counter -gt 9 ]; then
        file="./func_tests/data/neg_${counter}"
    fi
